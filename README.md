...
**microservice-ecom-docker-compose** is a sample microservice-based e-commerce application split into five independent services — Main, Product Catalog, Shopping Cart, User Profile, and Order Management — each containerized with its own Dockerfile. An Nginx reverse proxy routes traffic to the right service, and the whole stack is orchestrated with a single `docker-compose.yml` for easy local or EC2 deployment.
...
```bash
git clone https://github.com/damodar-dev/microservice-ecom-docker-compose.git
cd microservice-ecom-docker-compose
```
...
# Build Main Service
cd main-service
docker build -t damodar-main-service .
cd ..

# Build Product Catalog Service
cd product-service
docker build -t damodar-product-service .
cd ..

# Build Shopping Cart Service
cd cart-service
docker build -t damodar-cart-service .
cd ..

# Build User Profile Service
cd user-service
docker build -t damodar-user-service .
cd ..

# Build Order Management Service
cd order-service
docker build -t damodar-order-service .
cd ..
```
...
```bash
# Run Main Service
docker run -d -p 5001:80 --name damodar-main damodar-main-service

# Run Product Catalog Service
docker run -d -p 5002:80 --name damodar-product damodar-product-service

# Run Shopping Cart Service
docker run -d -p 5003:80 --name damodar-cart damodar-cart-service

# Run User Profile Service
docker run -d -p 5004:80 --name damodar-user damodar-user-service

# Run Order Management Service
docker run -d -p 5005:80 --name damodar-order damodar-order-service
```
